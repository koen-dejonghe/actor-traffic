...
import botkop.traffic.messaging.{CelltowerLocationMessage, MockLocationMessenger}
...
with WordSpecLike with Matchers with BeforeAndAfterAll with LazyLogging {
    ...
    "A CelltowerSupervisor" must {
        ...
        "find the closest celltower along the route" in {
            ...
            val mlm = new MockLocationMessenger(celltowerSupervisor)
            val vehicle = system.actorOf(VehicleActor.props(id, mlm, velocity, 250.milliseconds), name = "vehicle1")
            vehicle ! route
